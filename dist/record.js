...
async function main() {
    ...
    try {
        const state = await appendIteration(get("repo"), get("results-path"), get("state-path"), decision, get("metric-value"), normalizeResultStatus(get("verify-status") ?? "pass", "verify_status"), normalizeResultStatus(get("guard-status") ?? "skip", "guard_status"), get("hypothesis"), changeSummary, get("labels")?.split(" "), get("note"), get("iteration") ? parseInt(get("iteration")) : undefined);
        printJson(state);
    }
    catch (exc) {
